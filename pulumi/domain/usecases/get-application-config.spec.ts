...
describe("getApplicationConfig", () => {
  ...
  describe(".environment", () => {
    ...
    it("returns 'development' for other stack names", () => {
      expect(
        getApplicationConfig({ stackName: "dev", tag }).environment
      ).toEqual("development");
      expect(
        getApplicationConfig({ stackName: "foo-bar", tag }).environment
      ).toEqual("development");
      expect(
        getApplicationConfig({ stackName: "deps-lib-123.x", tag }).environment
      ).toEqual("development");
    });
  });
